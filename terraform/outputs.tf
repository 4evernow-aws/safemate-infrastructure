...
output "s3_static_hosting_url" {
  description = "S3 Static Website Hosting URL (Free Tier)"
  value       = "http://${aws_s3_bucket.static_hosting.bucket}.s3-website-${var.region}.amazonaws.com"
}
...
output "directory_api_url" {
  description = "URL for the Directory Creator API Gateway"
  value       = "https://${aws_api_gateway_rest_api.directory_api.id}.execute-api.${var.region}.amazonaws.com/${local.environment}"
}
...
# Email verification outputs removed - using Cognito directly for Free Tier compliance 