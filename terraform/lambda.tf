# =============================================================================
# SafeMate Lambda Functions Configuration
# =============================================================================
#
# This file configures all AWS Lambda functions for the SafeMate application.
# 
# Key Features:
# - Real Hedera testnet wallet integration
# - Secure key storage using AWS KMS and DynamoDB (Free Tier compliant)
# - No Secrets Manager usage (Free Tier compliant)
# - Email verification handled by Cognito directly (no custom Lambda)
...
# - No SES usage for email verification
# - Uses KMS + DynamoDB for secure storage
# - All Lambda functions optimized for Free Tier limits
#
# =============================================================================
...
resource "aws_lambda_layer_version" "hedera_dependencies_layer" {
  ...
  description      = "Layer containing Hedera SDK and AWS SDK dependencies - Updated to use cleaned layer"
...
resource "aws_lambda_function" "hedera_service" {
  ...
  environment {
    variables = {
      ...
      SAFEMATE_FOLDERS_TABLE     = aws_dynamodb_table.safemate_folders.name
      SAFEMATE_FILES_TABLE       = aws_dynamodb_table.safemate_files.name
...
# API Gateway resource for /onboarding/verify
resource "aws_api_gateway_resource" "onboarding_verify" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  parent_id   = aws_api_gateway_resource.onboarding_resource.id
  path_part   = "verify"
}
...
resource "aws_api_gateway_method" "onboarding_verify_post" {
  rest_api_id   = aws_api_gateway_rest_api.onboarding_api.id
  resource_id   = aws_api_gateway_resource.onboarding_verify.id
  http_method   = "POST"
  authorization = "NONE"  # No authorization required for email verification
}
...
resource "aws_api_gateway_method" "onboarding_verify_options" {
  rest_api_id   = aws_api_gateway_rest_api.onboarding_api.id
  resource_id   = aws_api_gateway_resource.onboarding_verify.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
# Lambda integration for email verification
resource "aws_api_gateway_integration" "onboarding_verify_integration" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_post.http_method
...
resource "aws_api_gateway_integration_response" "onboarding_verify_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_options.http_method
  status_code = aws_api_gateway_method_response.onboarding_verify_options_response.status_code
...
resource "aws_api_gateway_method_response" "onboarding_verify_post_response" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_post.http_method
  status_code = "200"
...
resource "aws_api_gateway_method_response" "onboarding_verify_options_response" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_deployment" "onboarding_deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.onboarding_verify.id,
      aws_api_gateway_method.onboarding_verify_post.id,
      aws_api_gateway_method.onboarding_verify_options.id,
    ]))
  }
  ...
  depends_on = [
    ...
    aws_api_gateway_method.onboarding_verify_post,
    ...
    aws_api_gateway_method.onboarding_verify_options,
    ...
    aws_api_gateway_integration.onboarding_verify_integration,
    ...
    aws_api_gateway_integration.onboarding_verify_options_integration,
    ...
    aws_api_gateway_integration_response.onboarding_verify_options_integration_response,
    ...
    aws_api_gateway_integration_response.onboarding_verify_post_integration_response,
    ...
    aws_api_gateway_method_response.onboarding_verify_post_response,
    ...
    aws_api_gateway_method_response.onboarding_verify_options_response,
...
resource "aws_api_gateway_deployment" "hedera_deployment" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      "cors-fix-v7-add-origin-headers-20250707",
    ]))
  }
...
resource "aws_api_gateway_integration" "onboarding_verify_options_integration" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_options.http_method
...
resource "aws_api_gateway_gateway_response" "onboarding_unauthorized" {
  ...
  response_parameters = {"gatewayresponse.header.Access-Control-Allow-Headers" = "'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,x-cognito-token'"
...
resource "aws_api_gateway_gateway_response" "onboarding_access_denied" {
  ...
  response_parameters = {"gatewayresponse.header.Access-Control-Allow-Headers" = "'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,x-cognito-id-token,x-cognito-access-token,Accept'"
    "gatewayresponse.header.Access-Control-Allow-Methods" = "'GET,POST,OPTIONS'"
    "gatewayresponse.header.Access-Control-Allow-Credentials" = "'true'"
  }
}
...
# Integration responses for folders POST method to pass through CORS headers
resource "aws_api_gateway_integration_response" "folders_post_integration_response" {
  ...
  depends_on = [aws_lambda_function.hedera_service]
}

# Integration responses for folders GET method to pass through CORS headers
...
# Integration responses for folders DELETE method to pass through CORS headers
...
resource "aws_api_gateway_gateway_response" "hedera_unauthorized" {
  ...
  response_parameters = {"gatewayresponse.header.Access-Control-Allow-Headers" = "'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token'"
...
resource "aws_api_gateway_integration_response" "onboarding_verify_post_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.onboarding_api.id
  resource_id = aws_api_gateway_resource.onboarding_verify.id
  http_method = aws_api_gateway_method.onboarding_verify_post.http_method
  status_code = aws_api_gateway_method_response.onboarding_verify_post_response.status_code

  depends_on = [aws_api_gateway_integration.onboarding_verify_integration]
}

# Integration responses for CORS

# =============================================================================
# Email Verification - REMOVED (Using Cognito Directly for Free Tier Compliance)
# =============================================================================
#
# Email verification is now handled directly by AWS Cognito using the built-in
# email sending capabilities. This approach is:
# - 100% Free Tier compliant
# - More reliable than custom Lambda functions
# - Simpler to maintain
# - Works for both new and existing users
#
# The frontend now uses Cognito's resendSignUpCode() function directly
# instead of calling a custom Lambda function via API Gateway.
#
# =============================================================================
